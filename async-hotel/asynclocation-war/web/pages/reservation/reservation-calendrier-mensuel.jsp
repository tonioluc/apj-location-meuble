...
<%
    try{
        ...
        reservation.EtatReservation eta=u.getEtatReservation(null,"ReservationPlanningAvecMere",debutEtFinDeSemaine[0],debutEtFinDeSemaine[1]);
        String[] listeDate=eta.getListeDate();
        String[] listeDayofmonth=eta.getListeDateDayOfMonth();
        String[] listIdChambre=eta.getListeIdVoiture();
        String[] listeChambre=eta.getListeVoiture();
        String[] numeroVoiture = eta.getNumeroVoiture();
        // URL du site
...
<div class="content-wrapper">
    ...
    <div class="week-nav">
        ...
        <span class="week-range" id="week-range"><%=mois%>/<%=annee%></span>
        <%--        <span class="week-range" id="week-range">Du <%=debutEtFinDeSemaine[0]%> au <%=debutEtFinDeSemaine[1]%></span>--%>
        ...
        <div class="row">
            <div class="col-xs-12 calendar-scroll">
                <table class="table table-bordered calendar-table">
                    <thead>
                    <tr>
                        <th class="room-col"> Voitures </th>
                        <th >T</th>
                        <% for(int i=0;i<listeDayofmonth.length;i++)
                        { %>
                        <th>
                            <%=listeDayofmonth[i]%>
                        </th>
                    ...
                    <tr>
                        ...
                        <% for(int j = 0; j < listeDate.length; j++) {
                            ReservationDetailsLib[] resaChambre = eta.getResaById(listeDate[j], listIdChambre[i], "AM");
                            int countResaOK = 0;
                            if(resaChambre != null && resaChambre.length > 0) { %>
                        <td>
                            <% for (ReservationDetailsLib r : resaChambre) {
                                if (r != null) {
                                    if("occupe".equals(r.getLibelleStatus())) countResaOK = 1;
                            %>
                            ...
                            <% if(countResaOK == 0) { %>
                            <div>
                                <a href="<%= lien %>?but=reservation/reservation-saisie.jsp&daty=<%= listeDate[j] %>&idproduit=<%= listIdChambre[i] %>&tranche=AM">
                    ...
                    <tr>
                        <td>PM</td>
                        <% for(int j = 0; j < listeDate.length; j++) {
                            ReservationDetailsLib[] resaChambre = eta.getResaById(listeDate[j], listIdChambre[i], "PM");
                            int countResaOK = 0;
                            if(resaChambre != null && resaChambre.length > 0) { %>
                        <td>
                            <% for (ReservationDetailsLib r : resaChambre) {
                            ...
                            <% if(countResaOK == 0) { %>
                            <div>
                                <a href="<%= lien %>?but=reservation/reservation-saisie.jsp&daty=<%= listeDate[j] %>&idproduit=<%= listIdChambre[i] %>&tranche=PM">
                                    <i class="fa fa-plus"></i>
                                </a>
                            </div>
                            <% } %>
