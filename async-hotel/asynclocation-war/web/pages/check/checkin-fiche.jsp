...
<% 
    try{
        ...
        pc.getChampByName("reservation").setLibelle("R&eacute;servation details");
        pc.getChampByName("idReservationMere").setLibelle("R&eacute;servation");
        pc.getChampByName("idReservationMere").setLien(lien+"?but=reservation/reservation-fiche.jsp&tab=inc/liste-checkin", "id=");
        ...
        pc.getChampByName("equivalence").setVisible(false);
        pc.getChampByName("etatlib").setLibelle("&Eacute;tat");
        pc.getChampByName("produitLibelle").setLibelle("Produit");
        pc.getChampByName("remarque").setLibelle("Remarque"); 
        pc.getChampByName("refproduit").setLibelle("Reference produit");
        pc.getChampByName("idtypelivraisonlib").setLibelle("Type de retour");
        pc.getChampByName("qte").setLibelle("Quantit&eacute;");
        pc.setTitre("Fiche de Livraison/R&eacute;cup&eacute;ration");
...
<%--                            <a class="btn btn-warning pull-left"  href="<%= lien + "?but="+ pageModif +"&id=" + id +"&acte=update"%>" style="margin-right: 10px">Modifier</a>--%>
                            ...
                            <a class="btn btn-success" href="<%= lien + "?but=apresTarif.jsp&acte=valider&id=" + id + "&bute=reservation/reservation-fiche.jsp&idresa="+t.getIdReservationMere()+"&classe="+classe+"&nomtable=checkIn&tab=inc/liste-checkin"%> " style="margin-right: 10px" onsubmit="closeModal()">Viser</a>
                            <% } %>
