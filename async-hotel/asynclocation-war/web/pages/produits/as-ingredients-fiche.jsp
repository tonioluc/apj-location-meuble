...
<%
    ...
    PageConsulte pc = new PageConsulte(a, request, u);//ou avec argument liste Libelle si besoin
    a = (Ingredients) pc.getBase();
    String isDispo=pc.getChampByName("etatlib").getValeur();
    pc.getChampByName("libelle").setLibelle("Produit");
    pc.getChampByName("unite").setLibelle("Unit&eacute;");
    pc.getChampByName("pu").setLibelle("Prix Unitaire");
    pc.getChampByName("durre").setLibelle("Dimension");
//    pc.getChampByName("quantiteparpack").setLibelle("Quantit&eacute; par pack");
    ...
    pc.getChampByName("filepath").setLibelle("Code barre");
    pc.getChampByName("filepath").setPhoto(true);

    //pc.getChampByName("filepath").setAutre("id='sary'");
    ...
    pc.getChampByName("idVoiture").setVisible(false);
    pc.getChampByName("actif").setVisible(false);
    pc.getChampByName("bienOuServ").setVisible(false);
    pc.getChampByName("compte_vente").setVisible(false);
    pc.getChampByName("compte_achat").setVisible(false);
    pc.getChampByName("tva").setVisible(false);
    pc.getChampByName("reference").setLibelle("R&eacute;ference");
    if (u.getUser().getIdrole().equalsIgnoreCase("achat")){
        pc.getChampByName("pu").setVisible(false);
        pc.getChampByName("pv").setVisible(false);
    }
    pc.setTitre("Fiche du produit");
    ...
    map.put("inc/checkout-liste.jsp", "");
    String tab = request.getParameter("tab");
    if (tab == null) {
        tab = "inc/checkout-liste.jsp";
    }
    map.put(tab, "active");
    try {
        // insertion recette
        ...
        pi.getFormu().getChamp("idingredients").setLibelle("Produit");
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        <%  if(isDispo.equals("DISPONIBLE")) { %> <a class="btn btn-danger pull-right"  href="<%=(String) session.getValue("lien") + "?but=apresTarif.jsp&acte=disponible&isdispo=false&id=" + request.getParameter("id")%>" style="margin-right: 10px">Indisponible</a><%}%>
                          <a class="btn btn-warning pull-right" href="<%= request.getContextPath() %>/DownloadProduit?id=<%= base.getId() %>" style="margin-right: 10px;">
                        T&eacute;l&eacute;charger le code barre <i class="fa fa-download"></i></a>
                        <a class="btn btn-info pull-right" href="<%= (String) session.getValue("lien") + "?but=pageupload.jsp&id=" + request.getParameter("id") + "&dossier=" + "Ingredients" + "&nomtable=ATTACHER_FICHIER&procedure=GETSEQ_ATTACHER_FICHIER&bute=" + pageActuel + "&classe=produits.Ingredients&id=" + request.getParameter("id") + "&nomprj="+ pc.getChampByName("libelle").getValeur() %>" style="margin-right: 10px;/*! display: block; *//*! margin: 5px auto; *//*! width: 111px; *//*! max-width: 111px; */">Attacher Fichier</a>
                        <a class="btn btn-info pull-right" href="<%= (String) session.getValue("lien") + "?but=stock/arrivage-saisie.jsp&idProduit=" + request.getParameter("id")%>" style="margin-right: 10px;">Saisie Arrivage</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-12 nopadding">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li class="<%=map.get("inc/checkout-liste.jsp")%>"><a href="<%= lien %>?but=<%= pageActuel %>&id=<%= a.getId() %>&tab=inc/checkout-liste.jsp">D&eacute;tails retour</a></li>
                </ul>
                <div class="tab-content">
                    <jsp:include page="<%= tab %>" >
                        <jsp:param name="id" value="<%= a.getId() %>" />
                    </jsp:include>
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const img = document.querySelector("img[src^='nullbarcode-image']");
        if (img) {
            img.src = contextPath + "/assets/" + img.src.replace(/^.*null/, "");
        }
    });
</script>