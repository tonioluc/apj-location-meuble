...
<%
    try {
        ...
        String classeMere = "stock.MvtStock",
                ...
                titre = "Saisie arrivage produit",
                redirection = "stock/arrivage-liste.jsp";
        ...
        String idreservation = request.getParameter("idresCheckin");
        String idreservation_checkout = request.getParameter("idresCheckout");
        MvtStock stockMere = null;
        if (idreservation != null && !idreservation.equalsIgnoreCase("")) {
            Reservation r = new Reservation();
            r.setId(idreservation);
            stockMere =  r.genererMvtStockSortie(null);
        }
        if (idreservation_checkout != null && !idreservation_checkout.equalsIgnoreCase("")) {
            Reservation r = new Reservation();
            r.setId(idreservation_checkout);
            stockMere =  r.genererMvtStockEntrer(null);
        }


        MvtStockArrivage mere = new MvtStockArrivage();
        ...
        pi.getFormu().getChamp("idobjet").setDefaut("ARRIVAGE");
        ...
        if (idProduit != null && !idProduit.isEmpty()){
            affichage.Champ.setDefaut(pi.getFormufle().getChampFille("idProduit"),idProduit);
        }
        ...
        if (stockMere != null)
        {
            pi.getFormu().setDefaut(stockMere);
        }
        MvtStockFilleLib[] mvfille = null;
        if (stockMere != null && stockMere.getFille().length > 0)
        {
            pi.setDefautFille(stockMere.getFille());
            mvfille =(MvtStockFilleLib[]) stockMere.getFille();
        }
