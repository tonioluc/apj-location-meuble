...
import stock.EtatStockMagasin;
import stock.EtatStockModele;
...
public class Magasin extends ClassMAPTable{
    ...
    public double getQuantiteProduit(Connection c, String idProduit) throws Exception {
        EtatStock eta = new EtatStock();
        EtatStock [] res = (EtatStock[]) CGenUtil.rechercher(eta,null,null,c," and idMagasin='"+this.getId()+"' and id='"+idProduit+"'");
        if(this.getId().compareToIgnoreCase("PNT000122")==0){
            //return 2;
        }
        if(res.length>0){
            return res[0].getReste();
        }
        return 0;
    }
