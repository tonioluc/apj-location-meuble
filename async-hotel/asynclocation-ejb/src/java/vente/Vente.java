...
protected EncaissementDetails [] generateDetailsEncaissements (Connection c ) throws Exception{
    ...
    public Object validerObject(String u, Connection c) throws Exception {
        ...
        try {
            ...
            if(this.getIdOrigine()!=null || this.getIdReservation()!=null){
                MvtCaisse mvtcaisse = new MvtCaisse();
                mvtcaisse.setIdOp(this.getIdReservation() == null ? this.getIdOrigine() : this.getIdReservation());
...
//            compta[i].setDebit((montantHT-retenue) * ((this.getTva()/100)));
    ...
    public VenteDetails[] getFilleVente(Connection c) throws Exception {
        boolean estOuvert = false;
        try {
            if (c == null) {
                estOuvert = true;
            ...
            VenteDetails temp = new VenteDetails();
            temp.setIdVente(this.getId());
            VenteDetails[] liste=(VenteDetails[]) CGenUtil.rechercher(temp, null, null, c, "");
            if (liste.length>0){
                return liste;
            }
            throw new Exception("Pas de facture detail pour ce facture");
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        } finally {
    ...
    public String rajoutRemarque(Connection c) throws Exception{
        Client cl=(Client) this.getFK("idClient",new Client(),null,c);
        return cl.getNom();
    }
...
//            String daty = this.getDaty().toString().replace("-", "");
            ...
            this.setNumfacture(dateForDisplay+"-"+cle.getNextVal());
