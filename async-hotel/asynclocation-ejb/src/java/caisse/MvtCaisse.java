...
public class MvtCaisse extends ClassEtat{
    ...
    public void setCredit(double credit) throws Exception {
        if(this.getMode().equals("modif")){
            if(credit<0){
                throw new Exception("Le Cr\u00E9dit doit \u00EAtre positif");
            }
        }
        this.credit = credit;
    }
    ...
    public ClassMAPTable createObject(String u, Connection c) throws Exception{
        ...
        this.checkMontant(c);
        this.checkMontantFact(c);
        MvtCaisse mvt = (MvtCaisse)super.createObject(u, c);
        if(this.getIdproforma()!=null && this.getIdproforma().compareTo("") != 0){
            validerObject(u,c);
        }
        super.validerObject(u,c);

        if(this.getIdOrigine().startsWith("CAU")){
            Caution[] cautions = (Caution[]) CGenUtil.rechercher(new Caution(), null, null, c, " and id = '"+this.getIdOrigine()+"'");
            if(cautions.length!=0){
                cautions[0].setDaty(this.getDaty());
//                cautions[0].setIdmodepaiement(this.getIdCaisse());
                cautions[0].updateToTableWithHisto(u, c);
            }
        }
        //(u, c);
        return mvt;
    ...
    public void checkMontantFact(Connection c)throws Exception{
        if(this.getIdOrigine()==null || this.getIdOrigine().isEmpty()){
            return;
        }

        VenteLib dp = (VenteLib) new VenteLib().getById(this.getIdOrigine(), "VENTE_CPL", c);
        if(dp!=null&&this.getCredit()>dp.getMontantreste()){
            throw new Exception("Le Cr\u00E9dit depasse le montant \u00E0 payer");
        }
    }

    public void checkMontant(Connection c)throws Exception{
        if(this.getIdOp()==null || this.getIdOp().isEmpty()){
            return;
        }
        ReservationLib reservationLib = (ReservationLib) new ReservationLib().getById(this.getIdOp(), "RESERVATION_LIB", c);
        if(reservationLib!=null&&this.getCredit()>reservationLib.getResteAPayer()){
            throw new Exception("Le Cr\u00E9dit depasse le montant \u00E0 payer");
        }
    }
    ...
    public ClassMAPTable validerObject(String u, Connection c) throws Exception{
        ...
        if(this.getIdproforma()!=null && this.getIdproforma().compareTo("") != 0){
            Proforma pr=enregistrerEtValider(u, c);     
            this.setIdOrigine(pr.getReservationPF().getVente().getId());
            this.updateToTableWithHisto(u,c);
        }
        super.validerObject(u, c);
        return this;
...
//        }
    ...
    public Proforma  enregistrerEtValider(String u,Connection c) throws Exception{
        //this.validerObject(u, c);
        Proforma rep = null;
        Proforma pr=new Proforma();
        pr.setId(this.getIdproforma());
        Proforma[] proformas = (Proforma[]) CGenUtil.rechercher(pr, null, null, c, "");
        if (proformas.length > 0) {
               proformas[0].validerObject(u, c);
               rep =  proformas[0];
        }
        return rep;
